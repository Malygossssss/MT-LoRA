...
_C.MODEL.MTLORA.ENABLE_TS_SHARED_COMPLEMENT = False
_C.MODEL.MTLORA.TS_SHARED_COMPLEMENT_GRAD_MODE = 'detach'
_C.MODEL.MTLORA.TS_SHARED_COMPLEMENT_EPS = 1e-5
_C.MODEL.MTLORA.TS_SHARED_COMPLEMENT_BETA = 1.0
_C.MODEL.MTLORA.ENABLE_PRINCIPAL_ANGLE_REG = False
_C.MODEL.MTLORA.PRINCIPAL_ANGLE_MODE = 'both'
_C.MODEL.MTLORA.PRINCIPAL_ANGLE_LAMBDA = 1e-3
_C.MODEL.MTLORA.PRINCIPAL_ANGLE_W_COL = 1.0
_C.MODEL.MTLORA.PRINCIPAL_ANGLE_W_ROW = 1.0
_C.MODEL.MTLORA.PRINCIPAL_ANGLE_START_EPOCH = 20
_C.MODEL.MTLORA.PRINCIPAL_ANGLE_WARMUP_EPOCHS = 10
_C.MODEL.MTLORA.PRINCIPAL_ANGLE_EPS = 1e-12
_C.MODEL.MTLORA.PRINCIPAL_ANGLE_REDUCTION = 'mean'
...
def update_config(config, args):
    ...
    config.MODEL.MTLORA.TS_SHARED_COMPLEMENT_GRAD_MODE = str(
        config.MODEL.MTLORA.TS_SHARED_COMPLEMENT_GRAD_MODE).lower()
    if config.MODEL.MTLORA.TS_SHARED_COMPLEMENT_GRAD_MODE not in {'detach', 'full'}:
        raise ValueError("MODEL.MTLORA.TS_SHARED_COMPLEMENT_GRAD_MODE must be one of: detach, full")

    config.MODEL.MTLORA.TS_SHARED_COMPLEMENT_EPS = float(
        config.MODEL.MTLORA.TS_SHARED_COMPLEMENT_EPS)
    if config.MODEL.MTLORA.TS_SHARED_COMPLEMENT_EPS <= 0:
        raise ValueError("MODEL.MTLORA.TS_SHARED_COMPLEMENT_EPS must be > 0")

    config.MODEL.MTLORA.TS_SHARED_COMPLEMENT_BETA = float(
        config.MODEL.MTLORA.TS_SHARED_COMPLEMENT_BETA)

    config.MODEL.MTLORA.PRINCIPAL_ANGLE_MODE = str(
        config.MODEL.MTLORA.PRINCIPAL_ANGLE_MODE).lower()
    if config.MODEL.MTLORA.PRINCIPAL_ANGLE_MODE not in {'col', 'row', 'both'}:
        raise ValueError("MODEL.MTLORA.PRINCIPAL_ANGLE_MODE must be one of: col, row, both")

    config.MODEL.MTLORA.PRINCIPAL_ANGLE_REDUCTION = str(
        config.MODEL.MTLORA.PRINCIPAL_ANGLE_REDUCTION).lower()
    if config.MODEL.MTLORA.PRINCIPAL_ANGLE_REDUCTION != 'mean':
        raise ValueError("MODEL.MTLORA.PRINCIPAL_ANGLE_REDUCTION must be 'mean'")

    config.MODEL.MTLORA.PRINCIPAL_ANGLE_LAMBDA = float(
        config.MODEL.MTLORA.PRINCIPAL_ANGLE_LAMBDA)
    config.MODEL.MTLORA.PRINCIPAL_ANGLE_W_COL = float(
        config.MODEL.MTLORA.PRINCIPAL_ANGLE_W_COL)
    config.MODEL.MTLORA.PRINCIPAL_ANGLE_W_ROW = float(
        config.MODEL.MTLORA.PRINCIPAL_ANGLE_W_ROW)
    config.MODEL.MTLORA.PRINCIPAL_ANGLE_START_EPOCH = int(
        config.MODEL.MTLORA.PRINCIPAL_ANGLE_START_EPOCH)
    config.MODEL.MTLORA.PRINCIPAL_ANGLE_WARMUP_EPOCHS = int(
        config.MODEL.MTLORA.PRINCIPAL_ANGLE_WARMUP_EPOCHS)
    config.MODEL.MTLORA.PRINCIPAL_ANGLE_EPS = float(
        config.MODEL.MTLORA.PRINCIPAL_ANGLE_EPS)

    if config.MODEL.MTLORA.PRINCIPAL_ANGLE_START_EPOCH < 0:
        raise ValueError("MODEL.MTLORA.PRINCIPAL_ANGLE_START_EPOCH must be >= 0")
    if config.MODEL.MTLORA.PRINCIPAL_ANGLE_WARMUP_EPOCHS < 0:
        raise ValueError("MODEL.MTLORA.PRINCIPAL_ANGLE_WARMUP_EPOCHS must be >= 0")
    if config.MODEL.MTLORA.PRINCIPAL_ANGLE_EPS <= 0:
        raise ValueError("MODEL.MTLORA.PRINCIPAL_ANGLE_EPS must be > 0")
    config.freeze()
